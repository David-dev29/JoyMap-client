            ...
            return (
              <div
                ...
                className={`flex items-center justify-between p-3 border rounded-lg cursor-pointer transition
                  ${selected ? 'border-rose-600 bg-rose-50' : 'border-gray-200 hover:border-gray-300'}`}
              >
                <div>
                  <h4 className="font-medium text-gray-900">{size.label}</h4>
                  <p className="text-sm text-gray-600">MXN {size.price.toFixed(2)}</p>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 ${selected ? 'border-rose-600 bg-rose-600' : 'border-gray-300 bg-white'} flex items-center justify-center`}>
